...
const OrdersTable = () => {
	...
	const [activeStatus, setActiveStatus] = useState(orderData.map(() => true));
	...
	const toggleActiveStatus = (index) => {
		setActiveStatus((prevStatus) => prevStatus.map((status, i) => (i === index ? !status : status)));
	};
						...
						{filteredOrders.map((order, index) => (
							...
							>
								...
								<td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>
									<span
										className={`px-2  rounded-full inline-flex text-xs py-1 font-semibold ${
								...
								<td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>
									<button
										...
										className='text-indigo-400 hover:text-indigo-300 ml-4 transition-transform active:scale-90'
									>
										{activeStatus[index] ? <Eye size={18} /> : <EyeOff size={18} />}
									</button>
								</td>
							</motion.tr>
						))}
